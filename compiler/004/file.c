...
Var *new_var(char *name, Type var_type, bool permanent)
{
    Var *new = calloc(1, sizeof(Var));
    new->name = name;
    new->type = var_type;
    ft_printf(out, "new var with type '%t'\n", var_type);
    if (permanent)
...
Var *get_var(char *name, bool permanent)
{
    if (permanent)
    {
...
Var *eval(Node *node)
{
    ...
    else if (node->token->type == add_ || node->token->type == sub_)
    {
        ...
        else if (left->type == characters_ && node->token->type == add_)
        {
            ...
            res->characters = calloc(ft_strlen(left->characters) + ft_strlen(right->characters) + 1, sizeof(char));
            ft_strcpy(res->characters, left->characters);
            ft_strcpy(res->characters + ft_strlen(res->characters), right->characters);
        }
        else
    ...
    else if (node->token->type == mul_ || node->token->type == div_)
    {
        ...
        if (left->type == integer_ || left->type == float_)
        {
            ...
            ft_printf(err, "Error in eval in '%t'\n", node->token->type);
    }
    else
...
void execute()
{
    ...
    */
    if (curr->token->type == assign_)
    {
        ...
        // exit(0);
        new_var(curr->left->token->value.name, curr->left->token->type, true);
        if (get_var(curr->left->token->value.name, true))
        {
            ft_printf(out, "found\n");
            exit(0);
        }
