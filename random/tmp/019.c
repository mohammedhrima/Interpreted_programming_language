...
struct
{
    ...
    {"func", func_dec_},
    // func attributes
    {"indexof", indexof_}, {"count", count_}, {"split", split_},
    {0, 0},
};
            ...
            if (fmt[i] == '%')
            {
                ...
                if (fmt[i] == 'k' || fmt[i] == 'v')
                {
                    ...
                    if (token)
                    {
                        ...
                        ft_printf(out, ", in index [%d]", token->value_index);
...
char *type_to_string(Type type)
{
    struct
    {
        ...
        // built in attributes
        {"indexof", indexof_},{"split", split_}, {"count", count_},
        // EOF
        {"EOF", eof_},
        {0, 0},
    };
...
Token *new_token(Type type)
{
    ...
    new->line = line;
    new->column = column;
    new->value_index = start - val_index;
    switch (type)
    {
    case identifier_:
    {
        ...
        if (type)
        {
            ...
            new->name = value;
        new->value_index += ft_strlen(value);
        break;
    }
    case characters_:
    {
        ...
        new->value_index += ft_strlen(new->characters) + 1;
        break;
    }
    case number_:
    {
    ...
    while (text[txt_pos])
    {
        ...
        for (int i = 0; operators_tokens[i].type; i++)
        {
            if (ft_strncmp(operators_tokens[i].name, text + txt_pos, ft_strlen(operators_tokens[i].name)) == 0)
            {
                ...
                token->value_index += ft_strlen(operators_tokens[i].name);
                break;
            }
        }
        if (type)
...
void skip(Type type)
{
    if (tokens[exe_pos]->type != type)
    {
        ...
        ft_putchar(out, '\n');
        print_space(tokens[exe_pos - 1]->value_index);
        ft_putstr(out, "^\n");
